...
public class GenericSpace implements NSpace, NepticalContext{
  ...
  GenericSpace(CursorProvider cursorProvider, String name, String... dataModules){
    this(cursorProvider, name, Stream.of(dataModules).map((moduleName) -> NSpaceManager.getDataModule(moduleName)).toArray(DataModule[]::new));
  }
  ...
  public void addData(String segment, NepticalData data) throws DataDefinitionException{
    ...
    if (dataModule == null) {
      throw new DataDefinitionException("NSpace " + getName() + " does not have a registered data module, call addData(Module, Segment, Data) to register and add this data");
    }
